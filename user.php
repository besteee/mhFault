...
'account_deposit', 'account_log', 'account_detail', 'act_account', 'pay', 'default', 'bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list','validate_email','track_packages', 'transform_points','qpassword_name', 'get_passwd_question', 'check_answer','faultDiy');
...
//猛虎故障自定义
if($action=='faultDiy'){
	$sql="SELECT * FROM".$GLOBALS['ecs']->table('fault_diy')."WHERE user_id='$user_id' and handle=1";
	$result=$GLOBALS['db']->getAll($sql);
	$arr=array();
	foreach($result as $key=>$row){
		$arr[$key]['goods_id']=$row['goods_id'];
		$arr[$key]['add_time']=date('Y-m-d',$row['add_time']);
		$arr[$key]['handle_time']=date('Y-m-d',$row['handle_time']);
		$goods_id=$row['goods_id'];
		$sql="SELECT * FROM".$GLOBALS['ecs']->table('goods')."WHERE goods_id='$goods_id' ";
		$goods=$GLOBALS['db']->getRow($sql);
		$arr[$key]['fault_name']=$goods['goods_name'];
		$arr[$key]['shop_price']=$goods['shop_price'];
	}

	$smarty->assign('fault_diy',$arr);
	$smarty->display('user_fault_diy.dwt');
}
//用户中心欢迎页
if ($action == 'default')
{
