 ...
 * ============================================================================
 * $Author: liuhui $
 * $Id: zxcomment_manage.php 17123 2010-04-22 07:28:54Z liuhui $
*/
...
if ($_REQUEST['act'] == 'list')
{
    ...
    assign_query_info();
    $smarty->display('zxcomment_list.htm');
}
...
if ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('zxcomment_list.htm'), '',
...
if ($_REQUEST['act']=='reply')
{
    ...
    /* 获取评论详细信息并进行字符处理 */
    $sql = "SELECT * FROM " .$ecs->table('zxcomment'). " WHERE comment_id = '$_REQUEST[id]'";
    ...
    /* 获得评论回复内容 */
    $sql = "SELECT * FROM ".$ecs->table('zxcomment'). " WHERE parent_id = '$_REQUEST[id]'";
    ...
    'href' => 'zxcomment_manage.php?act=list'));

    /* 页面显示 */
    assign_query_info();
    $smarty->display('zxcomment_info.htm');
}

/*修改用户发表时间*/
if ($_REQUEST['act']=='actionaddtime')
{
   /* 更新当前的评论状态为已回复并且可以显示此条评论 */
    $sql = "UPDATE " .$ecs->table('zxcomment'). " SET add_time  =  '" .local_strtotime($_POST[uadd_time]). "' WHERE comment_id = '$_POST[comment_id]'";
    $db->query($sql);
	/* 清除缓存 */
    clear_cache_files();
    ecs_header("Location: zxcomment_manage.php?act=reply&id=$_REQUEST[comment_id]&send_ok=$send_ok\n");
    exit;
...
if ($_REQUEST['act']=='action')
{
    ...
    /* 获得评论是否有回复 */
    $sql = "SELECT comment_id, content, parent_id FROM ".$ecs->table('zxcomment').
    ...
    {
        /* 更新回复的内容 */
        $sql = "UPDATE ".$ecs->table('zxcomment')." SET ".
               "email     = '$_POST[email]', ".
    ...
    {
        /* 插入回复的评论内容 */
        $sql = "INSERT INTO ".$ecs->table('zxcomment')." (comment_type, id_value, email, user_name , ".
    ...
    /* 更新当前的评论状态为已回复并且可以显示此条评论 */
    $sql = "UPDATE " .$ecs->table('zxcomment'). " SET status = 1 WHERE comment_id = '$_POST[comment_id]'";
    $db->query($sql);
...
if ($_REQUEST['act'] == 'check')
{
    ...
    {
        /* 允许评论显示 */
        $sql = "UPDATE " .$ecs->table('zxcomment'). " SET status = 1 WHERE comment_id = '$_REQUEST[id]'";
        $db->query($sql);
        ...
        ecs_header("Location: zxcomment_manage.php?act=reply&id=$_REQUEST[id]\n");
        exit;
    }
    else
    {
        /* 禁止评论显示 */
        $sql = "UPDATE " .$ecs->table('zxcomment'). " SET status = 0 WHERE comment_id = '$_REQUEST[id]'";
        $db->query($sql);
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $sql = "DELETE FROM " .$ecs->table('zxcomment'). " WHERE comment_id = '$id'";
    $res = $db->query($sql);
    if ($res)
    {
        $db->query("DELETE FROM " .$ecs->table('zxcomment'). " WHERE parent_id = '$id'");
    }

    admin_log('', 'remove', 'ads');

    $url = 'zxcomment_manage.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
if ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['checkboxes']))
    {
        ...
        {
            case 'remove':
                $db->query("DELETE FROM " . $ecs->table('zxcomment') . " WHERE " . db_create_in($_POST['checkboxes'], 'comment_id'));
                $db->query("DELETE FROM " . $ecs->table('zxcomment') . " WHERE " . db_create_in($_POST['checkboxes'], 'parent_id'));
                break;

           case 'allow' :
               $db->query("UPDATE " . $ecs->table('zxcomment') . " SET status = 1  WHERE " . db_create_in($_POST['checkboxes'], 'comment_id'));
               break;

           case 'deny' :
               $db->query("UPDATE " . $ecs->table('zxcomment') . " SET status = 0  WHERE " . db_create_in($_POST['checkboxes'], 'comment_id'));
               break;
        ...
        $link[] = array('text' => $_LANG['back_list'], 'href' => 'zxcomment_manage.php?act=list');
...
function get_comment_list()
{
    ...
    $sql = "SELECT count(*) FROM " .$GLOBALS['ecs']->table('zxcomment'). " WHERE parent_id = 0 $where";
    ...
    /* 获取评论数据 */
    $arr = array();
    $sql  = "SELECT * FROM " .$GLOBALS['ecs']->table('zxcomment'). " WHERE parent_id = 0 $where " .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        /* 标记是否回复过 */
//        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('zxcomment'). " WHERE parent_id = '$row[comment_id]'";
